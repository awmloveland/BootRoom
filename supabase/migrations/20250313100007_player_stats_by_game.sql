-- RPC: get player stats for a specific game
...
AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM game_members WHERE game_id = p_game_id AND user_id = auth.uid()) THEN
  ...
  player_aggregates AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE (pg.team = 'A' AND pg.winner = 'teamA') OR (pg.team = 'B' AND pg.winner = 'teamB'))::bigint AS won,
      COUNT(*) FILTER (WHERE pg.winner = 'draw')::bigint AS drew,
      COUNT(*) FILTER (WHERE (pg.team = 'A' AND pg.winner = 'teamB') OR (pg.team = 'B' AND pg.winner = 'teamA'))::bigint AS lost,
  ...
  recent_form AS (
    SELECT rf.name,
      string_agg(
        CASE
          WHEN (rf.team = 'A' AND rf.winner = 'teamA') OR (rf.team = 'B' AND rf.winner = 'teamB') THEN 'W'
  ...
  SELECT
    ...
    pa.times_team_a,
    pa.times_team_b,
    CASE WHEN pa.played > 0 THEN ROUND((pa.won::numeric / pa.played) * 100, 1) ELSE 0 END,
