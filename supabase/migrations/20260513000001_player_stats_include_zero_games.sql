-- Make get_player_stats and get_player_stats_public include roster players
-- who have not yet played any games. Previously both functions started from
-- player_aggregates (built from played weeks), so a roster row with zero
-- games was invisible — including admins who added a player from the new
-- Settings → Players "+ Add player" flow. After this migration, both
-- functions start from player_attributes and LEFT JOIN the aggregates, so
-- zero-game players surface with played/won/drew/lost = 0, qualified = false,
-- and points = 0.

-- ── get_player_stats (members) ────────────────────────────────────────────────
...
BEGIN
  ...
  SELECT
    attr.name,
    COALESCE(pa.played, 0)::bigint,
    COALESCE(pa.won, 0)::bigint,
    COALESCE(pa.drew, 0)::bigint,
    COALESCE(pa.lost, 0)::bigint,
    COALESCE(pa.times_team_a, 0)::bigint,
    COALESCE(pa.times_team_b, 0)::bigint,
    CASE WHEN COALESCE(pa.played, 0) > 0
      THEN ROUND((COALESCE(pa.won, 0)::numeric / pa.played) * 100, 1) ELSE 0 END,
    (COALESCE(pa.played, 0) >= COALESCE((SELECT min_games FROM config_vals LIMIT 1), 5)),
    (COALESCE(pa.won,  0) * COALESCE((SELECT win_pts  FROM config_vals LIMIT 1), 3) +
     COALESCE(pa.drew, 0) * COALESCE((SELECT draw_pts FROM config_vals LIMIT 1), 1) +
     COALESCE(pa.lost, 0) * COALESCE((SELECT loss_pts FROM config_vals LIMIT 1), 0))::bigint,
    attr.goalkeeper,
    attr.mentality,
    attr.rating,
    COALESCE(rf.form, '')
  FROM player_attributes attr
  LEFT JOIN player_aggregates pa ON pa.name = attr.name
  LEFT JOIN recent_form       rf ON rf.name = attr.name
  WHERE attr.game_id = p_game_id;
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_player_stats(uuid) TO authenticated;

-- ── get_player_stats_public (service role / public tier) ──────────────────────
