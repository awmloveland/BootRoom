-- Public variant of get_player_stats for use by the service role.
-- No membership check — access control is enforced at the Next.js layer
-- via the public_results_enabled and player_stats.public_enabled flags.
-- Granted only to service_role so it cannot be called by anon clients directly.
